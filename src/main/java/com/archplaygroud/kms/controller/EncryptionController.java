...
public class EncryptionController {
    ...
    public PathDTO createEncryptedPath(@PathVariable String companyId,
        ...
        return new PathDTO(encryptionService.encryptPath(companyId, pathDTO.getPath(), false));
    }

    @PostMapping(path = "/{companyId}/filePath/cached")
    @ResponseStatus(HttpStatus.CREATED)
    public PathDTO createEncryptedPathUsingCachedKey(@PathVariable String companyId,
                                       @RequestBody PathDTO pathDTO) {
        return new PathDTO(encryptionService.encryptPath(companyId, pathDTO.getPath(), true));
    }
}
